...
ZSH Tips and Cheatsheet by ZZapper
zzappers Tips Home
...
The Power of ZSH: Tutorial 01 : Return to Recent Directories (Z)
The Power of ZSH: Tutorial 02 : Global Aliases To Create Your Own Meta-Shell : Move Files
The Power of ZSH: Tutorial 03 : Global Aliases : Filters 
The Power of ZSH: Tutorial 04 : Selectively History Recall with your UP & DOWN Keys
...
ls *(.m+3)  # modified more than 3 days ago
...
echo chim
^chim^&-&-cheree      # reuse LHS with memory &
^str1^str2^:G         # replace as many as possible
^fred^joe^:s/sue/sid/ # all zsh modifiers apply *N*
!42:p
also use control-R
...
xy{t,y}(){ if [ "$0" = "xyy" ]; then echo run xyy code; else  echo run xyt code; fi ; echo run common code }
...
[[ 0 -eq 0 ]] && echo eq || echo neq
[[ 1 -eq 0 ]] && echo eq || echo neq
...
if [ "$p1" = "end" ] || [ "$p1" = "-e" ]
f=stuff.jpg ; if [[ $f =~ '\.(jpg|gif|png|jpeg)' ]] ;then echo 'ok' ;else echo 'nok' ;fi # test for image
...
fc -ln -4 > /tmp/hist   #  save last 4 history items to a file (without numbers) *N*
...
ls *.{jpg,gif}(.N) # dont break if one or other image type absent
...
mv *.*(^m-1) old/ # move all but todays files to sub-directory archive older files *N*
# exact dates (N)
ls -l *.*(mM4)
autoload -U age
ls -tl *.*(e-age 2021/02/01 now-)
ls -tl *(.e-age 2020/09/01 2020/01/01-)
ls -lt *(.om[0,5]e#age 2021-03-28 2021-04-04#) # 5 newest files in Mar/Apr *C*
mv *(.e#age 2017-10-01:00:00:00 2017-10-08:23:59:59#) /tmp # mv all Sept Files
grep -l 'server crash' *.log(e#age 2006/10/04:10:15 2006/10/04:12:45#) # *C*
...
file=/usr/tmp/fred5.txt
echo ${file##*/}                   # echo just the file name (strip the path)
echo ${file%/*.*}                  # echo just the path (strip the file name)
echo ${file%.*}                    # strip file extension
echo $file:r                       # strip file extension
echo ${file##*[^0-9]}              # strip all but trailing digit from filename $0 *C*
...
# Show all the .c files for which there doesn't exist a .o file.
print *.c(e_'[[ ! -e $REPLY:r.o ]]'_)

# Work with huge log directories?
# precise dates/times
touch -d '11:59 04 October 2006' fred.log
autoload -U age
ls -tl *.log(.e#age  2015/06/11 2015/06/13#)
...
alias lsimg=$'for f (*.(jpg|gif)) {printf $f;php -r "print_r(getimagesize(\'$f\'));" | grep width}' # 3 way quoting Ansi C quoting
#
...
for i (*.txt) { timeout 2s ./endoh1 < $i } # run a command in 2 second loop
# loop a command *N*
...
v='';repeat 6 {v="v$v";aptitude -$v moo;sleep 1}
...
if pgrep "tmux" > /dev/null ; then echo "Running" ; else echo "Stopped" ; fi # test process running
...
scp -rp *(om[1]) webuser@192.168.168.157:$PWD *N*
scp -rp *(.om[1]) webuser@111.111.111.111:${PWD/test/live}  # modify pwd remote path
...
# rsync
rsync -avz --update --existing *(.m0) backup/ # copy file if newer than target and target exists
rsync -vz --stats  --existing --update *(.m0)  webuser@111.11.111.122:$PWD # upload todays files if newer and existing *N*
rsync -rnvc --exclude '*.scc' root@155.19.138.100:${PWD/html/release}/ . # check for differences manipulate remote path
ls (x*~x[3-5])    # list files x* except x3 to x5
...
# a trick to allow parameters in an alias: alias ff='f(){F=$1; echo $F};f'
alias ytv='f(){F=$1;ffmpeg -i "$F" -af "volume=2" "loud$F"};f'
alias killp='f(){kill -9 $(ps aux | grep -i $1 | awk "{print $2}") E};f'
...
alias dxs='echo $(date)' # current date
dxs # dynamically re-evaluated always current date

# control structures templates *N*
pd()if (($#)){pushd "$@"} else {popd}
pd()p${${=:-op ush}[1+$#]}d $1  # obsfuscated
pd()$# $1;0()popd;1()pushd $1
pd()case $+1 in (0) popd;; (1) pushd "$1";; esac
pd()case $+1 {(0) popd ;; (*) pushd "$1" ;;}
function joe(){ [ $1 > 4 ] && echo big || echo small }
function fred(){ if [[ $1  =~ fred ]] { echo ok} else {echo nok} }
case "$1" in (|[A-Z]|V<->|G*);; (*) echo "Yes expand";; esac
if [[ $fred =~ "\bjoe\b|\bsid\b" ]] ; then  echo ok ; fi

# useful scripts
echo -n $((++cnt)) # increment a counter
echo ${(l:3::0:)$((++cnt))} # pad/format number
# display top line of files in date order
nl=0;for n (note???.txt(om)){echo -n $((++nl));head -n 1 $n}|less 
# power grep (not line based)
# list all c files containing all three keywords stdio,fcntl,mman anywhere in file 
has() { <$REPLY grep -q -- "$1" }
print -raC1 -- /usr/src/**.c(e*has stdio*e*has fcntl*e*has mman*)
###
